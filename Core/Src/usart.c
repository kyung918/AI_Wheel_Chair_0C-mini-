...
void DataGetFlowProcess(unsigned char port)
{
	...
	if (ptr->Buf[5] == 0xb7){
		...
		CommBufClear(port);
		break;	
	}else if (ptr->Ctr >= UART_MAXQ-1) { 				// 버퍼 ?��버플로우 처리
		CommBufClear(port);
		break;
	}
...
*/
	...
	//bAutoMode = (channels[8] == 0 ? true : false);
		...
		if((r_cPid == 0xae) || (r_cPid == 0xaf)) m_nPacketLen = 9;
		//pid 207(cf / Velocity)
		else m_nPacketLen = 13;
			...
			if(bAutoMode_Usart){
				...
				r_cPid = 0x00;
				if((fw == 0)&&(fV==0))DataSendToMotor(0x01, 0xae, 9);
			}
			else{
				if((fw == 0)&&(fV==0))DataSendToMotor(0x01, 0xae, 9);
...
//									ptr->Buf[7],ptr->Buf[8],ptr->Buf[9],ptr->Buf[10],ptr->Buf[11],ptr->Buf[12]);
		//if((ptr->Buf[ptr->Ctr-1] == 0x0d) || (ptr->Buf[ptr->Ctr-1] ==0x0a)){
			...
			if((cPid == 0xae) || (cPid == 0xaf)) m_nPacketLen = 9;
			//pid 207
			else if (cPid == 0xcf)m_nPacketLen = 13;
			else{
				...
				if ((bSerialType_Usart) || (!bAutoMode_Usart)){	
					...
					//ptr->Buf[7],ptr->Buf[8],ptr->Buf[9],ptr->Buf[10],ptr->Buf[11],ptr->Buf[12]);
						if(!(ptr->Buf[11] == 0x00)){
							if(!(ptr->Buf[12] == 0x00)){
								if(!(ptr->Buf[5] == 0x00)){
							    if(ptr->Buf[6] == 0xff) ptr->Buf[6] = 0xfe;
									if(ptr->Buf[9] == 0xff) ptr->Buf[9] = 0xfe;
									MOTOR_RS485_DIR_TXD;
									HAL_Delay(10);
									//printf("%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x\r\n",
									//ptr->Buf[0],ptr->Buf[1],ptr->Buf[2],ptr->Buf[3],ptr->Buf[4],ptr->Buf[5],ptr->Buf[6],
